...
class FlowContext {
  ...
  get session () {
    const sessionProp = this.options.sessionProperty
    this.ctx[sessionProp]._flow = this.ctx[sessionProp]._flow || {}
    return this.ctx[sessionProp]._flow
  }
  ...
  restart () {
    return this.current.startMiddleware()(this.ctx)
  }
  ...
  complete (result) {
    ...
    return this.current.resultMiddleware()(this.ctx)
  }
