...
flow.register = function (flow, handler) {
  if (!flow.name) {
    throw new Error("Can't find flow name")
  }
...
flow.middleware = function () {
  ...
  return function * (next) {
    ...
    this.startFlow = function (name, message, state) {
      debug('startFlow', name)
      if (!name || !allFlows[name]) {
        return Promise.reject("Can't find flow with name: " + name)
      }
      var flow = new Kwiz(allFlows[name].flow, state, self.answerHandlers)
